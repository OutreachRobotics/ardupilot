...
void ModeAltHold::run()
{
    ...
    reset_command = hal.rcin->read(CH_6) > RESET_REF_VALUE;
    ...
    if (counter>7){
        attitude_control->deleaves_controller_angVelHold_PD(lateral_input, pitch_input, yaw_input, thrust_input, motors->armed(), reset_command);
        counter=0;
    }
    counter++;
